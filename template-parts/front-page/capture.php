<div class="container bold narrow">
	<div class="row">
		<header class="">
			<h4>
				<div class="display-md centered">A Free Gift For You</div>
			</h4>
		</header>

		<div class="content">
			<div class="grid">

				<div>
					<header class="fz-md fw-semibold centered">
						Thanks for your interest.
					</header>
					<p>I'd love to work with you, but no matter what, your success is important to me.
					<p>Sign up for a free membership and I'll give you access to <strong>ten essential lessons</strong> that I know will help you – completely free. </p>

					<p>These are just a few of the topics you'll get:</p>
					<ul>
						<li>Dealing with criticism and being a perfectionist</li>
						<li>How to deal with stage fright</li>
						<li>How to deal with your fears and doubts about going after this</li>
						<li>How to find a good teacher</li>
					</ul>

					<p>By signing up, you'll also be the first to know about course updates, opportunities to meet with Jim, new free videos and more.</p>
					<div class="centered">
						<?php echo do_shortcode('[pmpro_signup level="1" short="emailonly" login="true" title="true" submit_button="Send Email" intro="false" custom_fields="true" redirect="/membership-account/"]'); ?>
						<?php // echo FrmFormsController::get_form_shortcode(array('id' => 7)); ?>
					</div>

				</div>

			</div>
		</div>
	</div>